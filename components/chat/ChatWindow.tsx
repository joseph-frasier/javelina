...
function renderInlineMarkdown(text: string, keyPrefix: string): ReactNode[] {
  ...
  return parts.map((part, idx) => {
    ...
    if (part.startsWith('`') && part.endsWith('`') && part.length >= 2) {
      return (
        <code key={key} className="px-1 py-0.5 rounded bg-surface-alt border border-border text-xs font-mono text-text">
          {part.slice(1, -1)}
        </code>
      );
    }
...
function renderAssistantMessage(content: string): ReactNode {
  ...
  return <div className="text-sm text-text">{nodes}</div>;
}
...
export function ChatWindow({ isOpen, onClose, orgId, tier, entryPoint }: ChatWindowProps) {
  ...
  return (
    <div
      ref={windowRef}
      className={`fixed bottom-20 right-4 sm:bottom-24 sm:right-6 w-[calc(100vw-2rem)] sm:w-[400px] h-[580px] max-h-[calc(100vh-8rem)] overflow-hidden rounded-2xl border border-border bg-surface text-text shadow-elevated flex flex-col z-50 transition-opacity duration-200 ${isTicketModalOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
    >
      ...
      <div className="relative flex items-center justify-between border-b border-border px-5 py-4">
        <div className="flex items-center gap-2">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-text-faint">
              Support Assistant
            </p>
            <h3 className="mt-0.5 text-base font-semibold tracking-tight text-text">Javi</h3>
          </div>
        </div>
        <button
          onClick={onClose}
          className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-surface-alt text-text-muted transition-colors hover:bg-surface-hover hover:text-text focus-visible:outline-none focus-visible:shadow-focus-ring"
      ...
      {/* Messages Area */}
      <div ref={messagesContainerRef} className="relative flex-1 space-y-4 overflow-y-auto bg-surface-alt p-5">
        {messages.map((message) => (
          <div key={message.id}>
            {message.role === 'assistant' ? (
              <div className="flex items-start">
                <div className="flex-1">
                  <div className="rounded-2xl rounded-tl-sm border border-border bg-surface px-4 py-3 text-text shadow-card">
                          ...
                          {message.citations.map((citation, idx) => (
                            <div key={idx} className="flex flex-col gap-0.5">
                              ...
                              {citation.lastUpdated && (
                                <span className="text-[10px] text-text-muted/70 text-text-faint">
                  ...
                  <p className="mt-1 ml-1 text-xs text-text-muted/70 text-text-faint">
              ...
              <div className="flex items-start gap-3 justify-end">
                <div className="flex-1 flex flex-col items-end">
                  <div className="max-w-[85%] rounded-2xl rounded-tr-sm bg-accent px-4 py-3 text-[#ffffff] shadow-md">
                    <p className="text-sm whitespace-pre-wrap">{message.content}</p>
                  </div>
                  <p className="mt-1 mr-1 text-xs text-text-muted/70 text-text-faint">
                    {message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
      ...
      {/* Input Area */}
      <div className="relative border-t border-border bg-surface p-4">
        <div className="relative">
          <textarea
            ...
            rows={2}
            className="min-h-[4.75rem] w-full resize-none overflow-y-auto rounded-xl border border-border bg-surface-alt px-4 py-4 pr-14 text-sm text-text placeholder:text-text-muted transition-all focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50"
        ...
        <p className={`mt-2 text-right text-xs ${inputValue.length >= MAX_MESSAGE_LENGTH ? 'font-medium text-accent' : 'text-text-muted/70 text-text-faint'}`}>
