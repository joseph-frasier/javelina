 ...
 * 
 * Step 3: Add Verification Endpoints (15 mins)
 *   - Add THREE new routes to routes/auth.js:
 *     1. POST /auth/resend-verification
 *     2. GET /auth/me/verification-status
 *     3. POST /auth/refresh-verification-status (NEW! Critical for email-verified page)
 ...
 * 
 * CRITICAL: The refresh-verification-status endpoint is essential for the
 * email-verified page to work. Without it, the frontend will be stuck in an
 * infinite loop because the session won't update after Auth0 verifies the email.
 */
 ...
 * ============================================================================
 * 
 * FILE: app.js or routes/auth.js (wherever you register Express routes)
 * 
 * WHAT TO DO: Add these THREE route handlers to your Express app
 * 
 * CRITICAL PATH NOTE: 
 * - The Next.js frontend adds /api prefix to all backend calls
 * - So these routes MUST be registered at /api/auth/* in your Express app
 * - Example: router.post('/api/auth/resend-verification', ...)
 * - NOT: router.post('/auth/resend-verification', ...)
 * 
...
/**
 * POST /auth/refresh-verification-status
 * Fetches current verification status from Auth0 and syncs to database/session
 * CRITICAL: This should be called when the user lands on /email-verified page
 * after clicking the verification link
 */
async function refreshVerificationStatus(req, res) {
  try {
    const auth0UserId = req.user.auth0UserId;
    const userId = req.user.userId;
    ...
    if (!auth0UserId) {
      return res.status(400).json({ 
        error: 'Not an Auth0 user',
        message: 'Cannot refresh verification status for non-Auth0 users' 
      });
    }
    
    console.log(`[REFRESH-VERIFICATION] Fetching status from Auth0 for user ${auth0UserId}`);
    ...
    // 2. Get user info from Auth0 Management API
    ...
    const emailVerified = userResponse.data.email_verified || false;
    console.log(`[REFRESH-VERIFICATION] Auth0 reports email_verified = ${emailVerified}`);
    ...
    if (updateError) {
      console.error('[REFRESH-VERIFICATION] Error updating database:', updateError);
      // Continue anyway - session update is more important
    } else {
      console.log(`[REFRESH-VERIFICATION] Database updated for user ${userId}`);
    }
    
    // 4. Update session
    req.session.user.emailVerified = emailVerified;
    ...
    console.log(`[REFRESH-VERIFICATION] Session updated for user ${userId}`);
    
    return res.json({ 
      success: true,
      email_verified: emailVerified,
      message: emailVerified ? 'Email verification confirmed!' : 'Email not yet verified'
    });
    
  } catch (error) {
    console.error('[REFRESH-VERIFICATION] Error:', error.response?.data || error);
    return res.status(500).json({ 
...
// IMPORTANT: Frontend calls these with /api prefix, so register them at /api/auth/*
// In your Express app setup (or wherever you register auth routes):

router.post('/api/auth/resend-verification', authenticateSession, resendVerification);
router.get('/api/auth/me/verification-status', authenticateSession, getVerificationStatus);
router.post('/api/auth/refresh-verification-status', authenticateSession, refreshVerificationStatus); // NEW!

// NOTE: If your /auth/me endpoint is at /auth (not /api/auth), that's fine - keep it there.
// These new verification endpoints need to be at /api/auth/* to match frontend calls.
