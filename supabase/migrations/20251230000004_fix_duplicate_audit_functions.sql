-- =====================================================
-- Drop and Recreate All Audit Logging Functions
-- =====================================================
-- This migration drops ALL existing audit functions and recreates them
-- to fix duplicate function definitions and ensure correct signatures
...
DROP FUNCTION IF EXISTS create_organization_with_audit(uuid, text, text);  -- Old version
...
CREATE FUNCTION create_zone_with_audit(
  p_user_id uuid,
  p_name text,
...
CREATE FUNCTION update_zone_with_audit(
  p_user_id uuid,
  p_zone_id uuid,
...
CREATE FUNCTION delete_zone_with_audit(
  p_user_id uuid,
  p_zone_id uuid
) RETURNS zones AS $$
DECLARE
...
CREATE FUNCTION create_zone_record_with_audit(
  p_user_id uuid,
  p_zone_id uuid,
  p_name text,
  p_type text,
...
CREATE FUNCTION update_zone_record_with_audit(
  p_user_id uuid,
  p_record_id uuid,
  p_name text DEFAULT NULL,
...
CREATE FUNCTION delete_zone_record_with_audit(
  p_user_id uuid,
  p_record_id uuid
) RETURNS zone_records AS $$
DECLARE
...
CREATE FUNCTION create_organization_with_audit(
  p_user_id uuid,
  p_name text,
  p_description text DEFAULT NULL,
...
CREATE FUNCTION update_organization_with_audit(
  p_user_id uuid,
  p_org_id uuid,
  p_name text DEFAULT NULL,
  p_description text DEFAULT NULL,
...
CREATE FUNCTION delete_organization_with_audit(
  p_user_id uuid,
  p_org_id uuid
) RETURNS organizations AS $$
DECLARE
