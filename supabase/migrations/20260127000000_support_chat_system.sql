...
CREATE TABLE kb_documents (
  ...
  is_stale boolean GENERATED ALWAYS AS (
    last_updated_at < (now() - interval '90 days')
  ) STORED,
...
COMMENT ON COLUMN kb_documents.is_stale IS 'Auto-computed: true if not updated in 90 days';
...
CREATE INDEX idx_kb_documents_stale ON kb_documents(is_stale, last_updated_at) 
  WHERE is_stale = true;
...
SELECT
  ...
  ARRAY_AGG(cm.content ORDER BY cm.created_at DESC LIMIT 5) as sample_messages
