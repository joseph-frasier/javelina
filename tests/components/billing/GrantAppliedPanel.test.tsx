...
describe('GrantAppliedPanel', () => {
  it('renders the summary and duration', () => {
    ...
    expect(screen.getByText('20% off plan')).toBeInTheDocument();
  ...
  it('says the discount is still applying when the Stripe sync is deferred', () => {
    ...
    expect(screen.getByText(/applying to your billing/i)).toBeInTheDocument();
  });
  ...
  it('labels the dismiss control with what dismissing actually does', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Redeem another code' })).toBeInTheDocument();
  });
  ...
  it('renders the one shared empty-grant headline', () => {
    ...
    expect(screen.getByText('Discount applied to this organization.')).toBeInTheDocument();
  });
});
